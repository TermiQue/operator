...
@tilelang.jit(pass_configs={tilelang.PassConfigKey.TL_DISABLE_WARP_SPECIALIZED: True})
def _moe_forward_kernel_all64_owner(
    hidden,
    intermediate,
    num_experts,
    total_padded_tokens,
    ...
    def kernel(
        ...
        with T.Kernel(num_blocks_m * 2, threads=threads) as bx:
            activation_fragment = T.alloc_fragment(
                (block_token, block_dhidden), dtype=dtype
            )
            weight_shared_0 = T.alloc_shared(
                (block_dexpert, block_dhidden), dtype=dtype
            )
            weight_shared_1 = T.alloc_shared(
                (block_dexpert, block_dhidden), dtype=dtype
            )
            accum_local_0 = T.alloc_fragment(
                (block_token, block_dexpert), dtype=accum_dtype
            )
            accum_local_1 = T.alloc_fragment(
                (block_token, block_dexpert), dtype=accum_dtype
            )
            accum_local_2 = T.alloc_fragment(
                (block_token, block_dexpert), dtype=accum_dtype
            )
            accum_local_3 = T.alloc_fragment(
                (block_token, block_dexpert), dtype=accum_dtype
            )
            routed_weight_local = T.alloc_fragment(
                (block_token,), dtype=dtype
            ...
            if actual_rows > 0:
                for gu_by in T.serial(
                    T.ceildiv(intermediate, block_dexpert * gate_pack)
                ):
                    T.clear(accum_local_0)
                    T.clear(accum_local_1)
                    T.clear(accum_local_2)
                    T.clear(accum_local_3)
                    for k in T.serial(T.ceildiv(hidden, block_dhidden)):
                        T.copy(
                            stacked_expert_tokens[
                                block_start : block_start + block_token,
                        ...
                        T.copy(
                            gate_w[
                                ...
                                gu_by
                                * gate_pack
                                * block_dexpert : (gu_by * gate_pack + 1)
                                * block_dexpert,
                                k * block_dhidden : (k + 1)
                                * block_dhidden,
                            ],
                        ...
                        T.copy(
                            gate_w[
                                expert_id,
                                (gu_by * gate_pack + 1)
                                * block_dexpert : (gu_by * gate_pack + 2)
                                * block_dexpert,
                                k * block_dhidden : (k + 1)
                                * block_dhidden,
                            ],
                    ...
                    for i, j in T.Parallel(block_token, block_dexpert):
                        if i < actual_rows:
                            up_logits[
                                block_start + i,
                                gu_by * gate_pack * block_dexpert + j,
                            ] = accum_local_2[i, j] * accum_local_0[i, j] * (
                                1.0
                                / (
                                    1.0
                                    + T.exp2(-accum_local_0[i, j] * scale)
                                )
                            )
                            up_logits[
                                block_start + i,
                                (gu_by * gate_pack + 1) * block_dexpert + j,
                            ] = accum_local_3[i, j] * accum_local_1[i, j] * (
                                1.0
                                / (
                                    1.0
                                    + T.exp2(-accum_local_1[i, j] * scale)
                                )
                            )
                ...
                for down_by in T.serial(
                    ...
                    for k in T.Pipelined(
                        ...
                        T.copy(
                            down_w[
                                ...
                                down_by
                                * down_pack
                                * block_dhidden : (down_by * down_pack + 1)
                                * block_dhidden,
                                k * block_dexpert : (k + 1)
                                * block_dexpert,
                            ],
                        ...
                        T.copy(
                            down_w[
                                expert_id,
                                (down_by * down_pack + 1)
                                * block_dhidden : (down_by * down_pack + 2)
                                * block_dhidden,
                                k * block_dexpert : (k + 1)
                                * block_dexpert,
                            ],
                    ...
                    for i, j in T.Parallel(block_token, block_dhidden):
                        if i < actual_rows:
                            out[
                                block_start + i,
                                down_by * down_pack * block_dhidden + j,
                            ] = accum_local_0[i, j] * routed_weight_local[i]
                            out[
                                block_start + i,
                                (down_by * down_pack + 1)
                                * block_dhidden
                                + j,
                            ] = accum_local_1[i, j] * routed_weight_local[i]
...
def _get_kernel(
    ...
    if kernel is None:
        kernel = _moe_forward_kernel_all64_owner(*key)
