...
def _moe_forward_kernel_all64(
    ...
    down_swizzle = 8
    gate_pack = 2
    down_block_token = 32 if hidden == 7168 else block_token
    down_threads = 256 if hidden == 7168 else threads
    down_tiles_per_parent = parent_token // down_block_token
    ...
    def kernel(
        ...
        with T.Kernel(
            num_blocks_m * down_tiles_per_parent,
            T.ceildiv(hidden, block_dhidden * down_pack),
            threads=down_threads,
        ) as (bx, by):
            up_shared = T.alloc_fragment(
                (down_block_token, block_dexpert), dtype=dtype
            )
            ...
            out_local_0 = T.alloc_fragment(
                (down_block_token, block_dhidden), dtype=accum_dtype
            )
            out_local_1 = T.alloc_fragment(
                (down_block_token, block_dhidden), dtype=accum_dtype
            )
            routed_weight_local = T.alloc_fragment(
                (down_block_token,), dtype=dtype
            )
            T.use_swizzle(down_swizzle)

            parent_bx = bx // down_tiles_per_parent
            tile_id = bx % down_tiles_per_parent
            expert_id = group_idx_for_bx[parent_bx]
            block_start = (
                parent_bx * parent_token + tile_id * down_block_token
            )
            padded_start = group_padded_offsets[expert_id]
            raw_start = group_offsets[expert_id]
            token_offset = block_start - padded_start
            actual_rows = T.max(
                ...
                T.min(
                    down_block_token,
                    group_sizes[expert_id] - token_offset,
                ),
            )
            ...
            if actual_rows > 0:
                T.clear(out_local_0)
                T.clear(out_local_1)
                for i in T.Parallel(down_block_token):
                    if i < actual_rows:
                        routed_weight_local[i] = routed_expert_weights[
                            raw_start + token_offset + i
                        ]
                for k in T.Pipelined(
                    ...
                    T.copy(
                        up_logits[
                            block_start : block_start + down_block_token,
                            k * block_dexpert : (k + 1) * block_dexpert,
                        ],
                        up_shared,
                    )
                ...
                for i, j in T.Parallel(down_block_token, block_dhidden):
                    if i < actual_rows:
                        out[
                            block_start + i,
