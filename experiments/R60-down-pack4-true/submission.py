...
def _moe_forward_kernel_all64(
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            out_local_2 = T.alloc_fragment(
                (block_token, block_dhidden), dtype=accum_dtype
            )
            out_local_3 = T.alloc_fragment(
                (block_token, block_dhidden), dtype=accum_dtype
            )
            routed_weight_local = T.alloc_fragment(
                (block_token,), dtype=dtype
            ...
            if actual_rows > 0:
                ...
                for k in T.serial(T.ceildiv(intermediate, block_dexpert)):
                    T.copy(
                        up_logits[
                            block_start : block_start + block_token,
                            k * block_dexpert : (k + 1) * block_dexpert,
                ...
                for i, j in T.Parallel(block_token, block_dhidden):
                    if i < actual_rows:
                        ...
                        out[
                            block_start + i,
                            (by * down_pack + 2) * block_dhidden + j,
                        ] = out_local_2[i, j] * routed_weight_local[i]
                        out[
                            block_start + i,
                            (by * down_pack + 3) * block_dhidden + j,
                        ] = out_local_3[i, j] * routed_weight_local[i]
