...
@tilelang.jit(pass_configs={tilelang.PassConfigKey.TL_DISABLE_WARP_SPECIALIZED: True})
def _moe_forward_kernel_all64_tail32(
    hidden,
    intermediate,
    num_experts,
    total_padded_tokens,
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            up_shared = T.alloc_fragment(
                (tail_block_token, block_dexpert), dtype=dtype
            )
            down_shared_0 = T.alloc_shared(
                (block_dhidden, block_dexpert), dtype=dtype
            )
            ...
            out_local_0 = T.alloc_fragment(
                (tail_block_token, block_dhidden), dtype=accum_dtype
            )
            out_local_1 = T.alloc_fragment(
                (tail_block_token, block_dhidden), dtype=accum_dtype
            )
            routed_weight_local = T.alloc_fragment(
                (tail_block_token,), dtype=dtype
            )
            T.use_swizzle(down_swizzle)
            ...
            sub_tile = bx % 4
            m64_tile = sub_tile // 2
            half_tile = sub_tile % 2
            expert_id = group_idx_for_bx[parent_bx]
            m64_start = parent_bx * parent_token + m64_tile * block_token
            padded_start = group_padded_offsets[expert_id]
            raw_start = group_offsets[expert_id]
            m64_token_offset = m64_start - padded_start
            m64_actual_rows = T.max(
                0,
                T.min(
                    block_token,
                    group_sizes[expert_id] - m64_token_offset,
                ),
            )
            block_start = m64_start + half_tile * tail_block_token
            token_offset = (
                m64_token_offset + half_tile * tail_block_token
            )
            actual_rows = T.max(
                0,
                T.min(
                    tail_block_token,
                    m64_actual_rows - half_tile * tail_block_token,
                ),
            )

            if m64_actual_rows > 0:
                if m64_actual_rows < block_token:
                    if actual_rows > 0:
                        T.clear(out_local_0)
                        T.clear(out_local_1)
                        for i in T.Parallel(tail_block_token):
                            if i < actual_rows:
                                routed_weight_local[i] = (
                        ...
                        for i, j in T.Parallel(
                            tail_block_token, block_dhidden
                        ):
                            if i < actual_rows:
                                out[
                                    block_start + i,
...
def _get_kernel(
    ...
    if kernel is None:
        if int(hidden) == 7168:
            kernel = _moe_forward_kernel_all64_tail32(*key)
        else:
