...
@tilelang.jit(pass_configs={tilelang.PassConfigKey.TL_DISABLE_WARP_SPECIALIZED: True})
def _moe_forward_kernel_all64_raster_gsm2(
    hidden,
    intermediate,
    num_experts,
    total_padded_tokens,
    ...
    gate_pack = 2
    group_size_m = 2
    gate_grid_m = num_blocks_m * 2
    gate_grid_n = (
        intermediate + block_dexpert * gate_pack - 1
    ) // (block_dexpert * gate_pack)
    down_grid_m = num_blocks_m * 2
    down_grid_n = (
        hidden + block_dhidden * down_pack - 1
    ) // (block_dhidden * down_pack)
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            group_width = group_size_m * gate_grid_n
            group_id = pid // group_width
            first_bx = group_id * group_size_m
            effective_group_m = T.min(
                group_size_m, gate_grid_m - first_bx
            )
            pid_in_group = pid % group_width
            bx = first_bx + pid_in_group % effective_group_m
            by = pid_in_group // effective_group_m
        ...
        with T.Kernel(
            ...
            group_width = group_size_m * down_grid_n
            group_id = pid // group_width
            first_bx = group_id * group_size_m
            effective_group_m = T.min(
                group_size_m, down_grid_m - first_bx
            )
            pid_in_group = pid % group_width
            bx = first_bx + pid_in_group % effective_group_m
            by = pid_in_group // effective_group_m
...
def _get_kernel(
    ...
    if kernel is None:
        kernel = _moe_forward_kernel_all64_raster_gsm2(*key)
