...
[综合](https://xpuoj.com/d)

代码评测指南

4 条回复

[![img](https://gravatar.com/avatar/76db96653591df2b0b39f3c39e9eda85?size=80&default=404)](https://xpuoj.com/u/ceerrep)

[ceerrep](https://xpuoj.com/u/ceerrep)

发表于 2026-04-03 10:03:27已编辑
...
3. **测速**：再连续执行若干次（通常 2000 次，算力密集的题目按规模递减）。GPU 端使用 `cudaEvent` 测时，取**平均值**作为 T_k。
...
入口函数能用的 `torch.*` 是**白名单制**——所有访问都经过一个 torch 代理对象。详见 [PyTorch 接口白名单](https://xpuoj.com/d/3)。简要分类：
